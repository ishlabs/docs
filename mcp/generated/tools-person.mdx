...
``person_ids``, which you pass as ``audience.person_ids`` on ``study_run``
/ ``ask_run``.
...
**List mode** (no ``person_id``): people in the workspace plus the
...
own; required ahead of ``person_attach``), ``"shared"``
