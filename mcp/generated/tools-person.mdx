...
when you run the server yourself. Full rules below.
...
exclusive with every generation input; contract below.
...
`employment_status_in`: extended demographic enums; legal values below.
...
``scope="all"`` orders every pool workspace → platform → shared.
``visibility="workspace"`` (the caller's own people) is what
``person_attach`` requires ahead of itself; ``"shared"`` is
community-published and ``"platform"`` admin-curated.
