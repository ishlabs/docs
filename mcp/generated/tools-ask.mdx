...
`wait`: bool, default ``False`` - returns immediately with a ``next_action``
pointing at ``ask_get`` to poll round 1. ``True`` blocks until round 1 is
terminal (summary included), or returns the in-flight envelope on ``timeout``.
`timeout`: float seconds, default 240 - under the ~300 s idle limit at which
Claude Code kills a silent call; on elapse the envelope carries the ids and a
``next_action``. Blocking contract:
