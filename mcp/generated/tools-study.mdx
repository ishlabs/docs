...
- ``workspace_id`` → **browse**: paginated list of every study in that
  workspace, each row carrying alias, name, modality, ``status`` /
...
(default 20; use 5 for a smoke probe). Credits: one per completed
participant. ``wait`` / ``timeout`` as on ``study_run``.
...
2. Pick the studies worth reading from the returned ``studies`` rows
   ...
   ``status`` / ``participant_count`` / ``updated_at``; rows carry the alias
   only, the UUID comes with ``study_get``).
