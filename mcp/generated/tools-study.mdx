...
Read the results with ``study_comparison(source_study_id)`` once the
...
2. Pick the studies worth reading from the returned ``studies`` rows
   (each carries ``id`` / ``alias`` / ``workspace_id`` / ``name`` /
   ``modality`` / ``status`` / ``participant_count`` / ``updated_at``).
...
of 3 after ``study_create`` and ``study_add_iteration``: resolve an audience
