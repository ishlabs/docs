...
document; needs ``modality``. Exclusive.
...
so far come back so you can continue with the primitives. Argument errors
raise before anything is created.

**Returns** ``StudyLaunchResponse``: ``study``, ``iteration``, ``run``,
``next_action``, plus ``notes`` and ``suggestions``. Full shape:
``docs_get("reference/responses")``.
...
create participants, dispatch the modality-appropriate batch.
...
available for chat studies. A failed analysis does NOT fail the run:
``analysis_error`` carries the reason and ``next_action`` the retry.
