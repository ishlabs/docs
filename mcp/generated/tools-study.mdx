...
- ``interactive``: `url`, `platform` (browser / android / figma / code),
  ...
  `start_node_id`. The ``url`` must load WITHOUT your session - see
  "Reaching your URL" below.
...
- ``text``: `content_md` - markdown body (inline, or ``@file.md``), the
  ...
  (inline, or ``@file.html``) is the paste-exact branded-email carrier. The
  body fields are mutually exclusive: markdown OR html, not both. Optional
  `sender_name` / `sender_email`, `featured_image_url`, `segmentation`,
...
same shape as ``chatbot_test``. Concept: ``docs_get("concepts/iteration")``.
...
study; later runs reuse the iteration. Credentialed pages: register a login
once with ``site_access_set``. Builder clients get a tailored ``notes`` hint
on the response.
...
benchmarking, see ``study_benchmark``. Concept:
...
**Returns** ``PaginatedList[Study]`` (browse), ``Study`` (``full``),
...
- ``study_id`` → single-study results, shaped by ``view``:
  ...
  runs; ``&#123;latest, history}`` where ``latest`` is the most recent analysis
  and ``history`` holds the EARLIER ones only, newest-first - it no longer
  repeats ``latest``, so a study analyzed once returns ``history: []``).
...
``study_add_iteration`` → ``study_run`` chain).

Pass exactly one of ``url`` / ``content_md`` / ``content_url``.

`name`: human-readable study name.
`people`: required - a fresh study has no panel to reuse. ``&#123;"sample": N}``
(this workspace's people), ``&#123;"person_ids": [...]}``, or ``&#123;"generate":
&#123;"description": ..., "count": N}}`` to mint new people first.
`workspace_id`: UUID or alias (e.g. ``w-6ec``). Exclusive with ``workspace_name``.
`workspace_name`: case-insensitive; an ambiguous name is a
``[validation_error]`` naming the candidates.
`url`: interactive - the public page / prototype participants visit; must
load WITHOUT your session.
...
and text modalities unless every resolved person already has one.
`max_interactions`: int, default 20 - per-participant step cap; 5 for a
smoke probe. Bounds credits, not just duration.
`screen_format`: ``url`` only - ``desktop`` (default) | ``mobile_portrait``.
`analyze`: bool, default ``False``. With ``wait=True``, runs
``study_analyze`` and returns it in ``run.analysis``.
`wait`: bool, default ``False``; ``True`` blocks until the run is terminal.
`timeout`: float seconds, default 240. Both as on ``study_run``.

**Credit cost:** per successful participant ``max(1, round(steps * rate))``
- interactive 1 credit a step, media 0.1-0.5 - so ``max_interactions``
bounds spend; subscription-funded, no per-dispatch approval, pre-flight
failures cost zero (``docs_get("reference/credits")``).

Use the primitives for chat studies, Figma files, multiple iterations, or
edits. ``task`` becomes the study's one assignment; ``questions`` the
...
substring one, rather than a guess. ``screen_format`` is ignored for
...
share-preview link, or a login registered with ``site_access_set``) are
written out there.
...
inline. ``None`` reuses the iteration's panel.
`config_id`: simulation-config override (alias ``c-...``). Required for media
and chat studies unless every resolved person already has one assigned;
optional for interactive. Pair-chat note below.
`dispatch`: bool, default ``True``. ``False`` stages the participants
without drawing credits - a review checkpoint; run later with
``people=None``.
`wait`: bool, default ``False``. ``True`` polls every participant to
terminal or ``timeout``; ``False`` returns the post-dispatch snapshot with a
``next_action`` poll hint.
...
a ``next_action``. Read the client-ceiling caveat below before raising it.
`max_interactions`: int, default 20 - per-participant step cap (interactive
/ media). Bounds SPEND, not just duration. Ignored for chat. Must be >= 1.
`max_turns`: int - per-participant turn cap (chat / chat_pair). Omitted →
backend uses request > iteration's stored value > 14. Ignored otherwise.
Must be >= 1.
`analyze`: bool, default ``False``. With ``wait=True``, runs
...
available for chat studies. Failure contract below.
...
Blocking contract: ``docs_get("reference/long-running-jobs")``.
