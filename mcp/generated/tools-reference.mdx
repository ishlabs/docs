...
A saturated account refuses the create; reuse an existing reference
instead.
...
**Returns** ``Reference``. Full shape: docs_get("reference/responses").
...
**Returns** ``&#123;"deleted": True, "reference_id": &lt;id>}``.
...
**Returns** ``PaginatedList[Reference]`` (no id) or ``Reference`` (with
id). Full shape: docs_get("reference/responses").
