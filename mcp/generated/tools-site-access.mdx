...
surfaces as ``None`` rather than erroring. ``login`` has no bound origin.
...
``public_affirmed``) and, where one is bound, ``origin``.
...
``cleared_public_affirmation`` is always ``False`` for ``public``.

**Returns** ``&#123;"method": &lt;method>, "configured": True, "origin": str | None,
"cleared_public_affirmation": bool}``.
