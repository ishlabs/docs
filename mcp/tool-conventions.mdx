...
| Part | Values |
| --- | --- |
| Noun | `workspace`, `reference`, `study`, `ask`, `person`, `chatbot`, `simulation`, `site_access`, `upload`, `connect`, `me`, `docs` |
...
[`reference_get`](/mcp/generated/tools-reference), [`study_get`](/mcp/generated/tools-study),
...
<Warning>
  ...
  `study_delete`, `ask_delete`, `reference_delete`, `person_delete`, `chatbot_delete`,
...
| --- | --- | --- | --- |
| workspace, reference | `w-` | ask | `a-` |
| study | `s-` | ask round | `r-` |
| iteration | `i-` | chatbot endpoint | `ep-` |
| person | `p-` | chatbot configuration | `cc-` |
| participant | `pt-` | frame | `f-` |

An id parameter validates the prefix: pass a `w-` workspace alias where a study id is expected
and it raises `[validation_error]` before any backend call. A reference resolves through
the workspace prefix, since a reference is a workspace underneath.
