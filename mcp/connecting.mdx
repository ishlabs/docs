...
<Tip>
  Point the CLI at a different endpoint (a local backend or a preview environment) by
  setting `ISH_MCP_URL` before any `ish mcp` command. `ish mcp list` reads the same
  variable, so reported status reflects the override.
