...
```mermaid
flowchart TD
  D["dispatch (wait=False)"] --> R["response: IDs + next_action"]
  R --> W["do other work"]
  W --> P["call next_action.tool<br/>after check_after_seconds"]
  P --> C{"value at status_path<br/>in terminal_statuses?"}
  C -->|"no"| BO["back off, then poll again"]
  BO --> P
  C -->|"yes"| T["terminal: read the result"]
```
