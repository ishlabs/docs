...
Both are **CLI capabilities**: they run on your own machine. The hosted [MCP server](/mcp/overview) has no path to your `localhost`, so when an agent drives a cloud run against a local build, the `ish connect` tunnel still has to come from the CLI on your machine.
...
```mermaid
flowchart LR
  subgraph local["study run --local"]
    direction LR
    PW["Playwright browser<br/>on your machine"] --> LH1["localhost:3000"]
  end

  subgraph connect["ish connect"]
    direction LR
    CB["cloud browser"] --> TUN["Cloudflare tunnel<br/>(*.trycloudflare.com)"]
    TUN --> LH2["localhost:3000<br/>on your machine"]
  end
```

In the local flow the browser and your app sit on the same machine, so nothing leaves the laptop. In the connect flow the browser runs in the cloud and reaches back through the tunnel.
