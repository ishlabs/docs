...
Generation builds new AI People; it does not turn the output into a real human. A person built from a real transcript is closer to the mark than one built from a prompt alone, but it is still simulation. Frame the findings the same way.
...
- A brief, a source, or both. You need at least one. A brief alone produces a plausible Person; a brief plus a source pins it to a reaction you have already seen.
...
The fastest path: describe who you are building for and let ish build matching people. Generation runs as an async job, so the command enqueues it and waits (typically 30 to 60 seconds), then prints the resulting people.
...
A source is the difference between a plausible Person and one tied to an actual reaction. Attach an interview transcript, a support email, a screenshot, or a call recording, and the generated person reflects the vocabulary, objections, and friction in that file.

<Steps>
<Step title="Point generation at a source">
Pass a local file straight to generation. The CLI uploads and processes it inline before building the Person; MCP does the same through `source_paths`.
...
The note captures how a real person reacted to that file, and it rides along to generation so the generated Person reflects the reaction, not just the document. In the CLI it is `--source-description`, paired with each `--source` by position. In MCP it is `reaction_notes`, one entry per `source_paths` file in the same order; use `""` to skip a note for a file. The note is capped at 500 characters.
</Step>

<Step title="Read the result">
Generation prints the people it built. When sources are attached it also produces grounded scenarios: recorded reactions the Person carries into a simulation as past context. Pass `--no-scenarios` (CLI) or `generate_evidence=False` (MCP) to skip them.
</Step>
</Steps>
...
study_run(study_id="s-1a2", audience={"person_ids": ["p-d4e"]})
```
</CodeGroup>

To pin generation to exactly one Person for a pair-chat rehearsal, generate with a count of one per side and feed each returned id into the chat configuration.
...
| One Person crafted through an iterative probe loop | [Build a person from notes](/cli/generated/person) (`suggest-scenarios` + `evidence`) |
| An exact Person from a JSON spec, no model involved | `ish person create --file` |
| People that already exist in a pool | Sample from the pool ([people and panels](/concepts/people)) |
...
<Columns cols={2}>
  ...
  <Card title="MCP: person tools" href="/mcp/generated/tools-person" icon="/images/logos/mcp.svg">
    The full `person_generate` and `source_upload` parameter contract.
  </Card>
  <Card title="People and panels" href="/concepts/people" icon="users">
