...
group_b = [p-ccc, p-ddd]
      ...
      (p-aaa <-> p-ccc)  (p-bbb <-> p-ddd)
```

Equal counts zip 1:1. One side of exactly one **broadcasts** to the other: `[p-aaa]` against `[p-ccc, p-ddd]` makes two conversations that share `p-aaa`. That is the 1xN rehearsal, fix one side and vary the other. Any other mismatch is rejected.
