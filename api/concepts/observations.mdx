---
title: "Observations"
description: "Why the persona perceives only human-sensory renders, and why action labels must not leak hidden state."
mode: "explanation"
---

Each turn you send an **observation**: what the persona perceives right now. The API restricts observations to human-sensory renders, the things a person sitting in front of your environment would take in through their eyes and ears. This restriction is the load-bearing choice that keeps the product honest.

## Human-sensory only

An observation is a list of blocks, and only two kinds exist today:

- `image`: a rendered frame, a screenshot of what the persona sees. Available now.
...
Feeding the persona structured state (a scene graph, game-state JSON, a chess FEN, a DOM or accessibility-tree dump) is the easier engineering path, and it is precisely the path ish does not take.

ish answers one question: **how would this particular human behave here.** That answer is only valid if the persona perceives what a human perceives, with the same ambiguity, occlusion, and room to misread. A persona handed the true state is no longer simulating a human; it is a solver wearing a persona's name, and its behavior stops predicting a real person's.
...
- An adventure-game hotspot named `rusty key` tells the persona the object's identity and narrative role before it has even looked.

Declare actions by visible affordance instead: a bare target and a generic verb (`move`, `click`, a from-and-to square pair). The persona reads the *meaning* off the pixels. Option names that are printed on the screen are fine in an enum, because they are already visible; what a control *does* must stay discoverable from the frame. The practical rules are in [author your environment](/api/guides/environment-authoring).
