...
A natural ending takes precedence over budget truncation, so a persona that finishes on the last allowed turn reports `completed`, never `max_turns`. Session status is an open union: tolerate a status you do not recognize rather than failing on it.
