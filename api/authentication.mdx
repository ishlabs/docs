...
A key carries any combination of four scopes. Grant only what the integration needs.
...
A new key is minted with the two `sessions:*` scopes. The `environments:*` pair is not granted by default — ask for it when the integration manages the registry rather than just running against it.
