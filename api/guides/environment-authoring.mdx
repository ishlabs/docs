...
description: "Declare actions the persona can pick, write labels that do not leak, guide the resolver, and render frames that decode."
mode: "how-to"
---

Your environment holds up two things each turn: the frame the persona perceives and the actions it may pick from. This guide covers how to declare actions, keep labels honest, guide the resolver in intent mode, and render frames that survive the wire. For the reasoning behind the rules, see [observations](/api/concepts/observations) and [the decide-only loop](/api/concepts/decide-only-loop).

## Declare actions

Each turn you send `valid_actions`, the set available right now. The set can change every turn as the persona moves through your environment. A declared action has this shape:
...
A label names what is *pressable*, never what it *means*. The persona reads meaning off the pixels, so a label that encodes hidden state hands it knowledge a person would not have.
...
- Declare `click` on a visible target, not `rusty key` (which leaks an object's identity and role before the persona has looked).

Your on-screen control hints also shape the persona's intents, because it reads them literally. If your UI overlays "press the up arrow key", the persona will intend exactly that. Write control labels the way you want intents phrased. The reasoning is in [observations](/api/concepts/observations#the-label-leak-rule).

## Guide the resolver (intent mode)

In intent mode a resolver maps the persona's intent onto your declared actions. Two layers let you steer it, and **neither is ever shown to the persona**:
...
```js
meta()              // -> { name, platform, pacing }   matches your create-session environment and pacing
...
state()             // -> { ...your ground truth... }   OPTIONAL; for your own scoring only; NEVER sent to the persona
```
...
<Warning>
  `state()` is for your own scoring and must never reach the persona. Sending it as an observation defeats the whole point of running the persona. See [score outcomes](/api/guides/scoring-outcomes).
</Warning>
