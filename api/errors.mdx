...
A session can only be opened for a person the workspace owns or one published in the platform library. HUMAN People, the records a real person owns, are out of scope by policy. Use an in-scope `person_id`.
...
### task_in_published_listing {#task_in_published_listing}
...
### search_rule_invalid {#search_rule_invalid}
...
### search_rule_too_complex {#search_rule_too_complex}
