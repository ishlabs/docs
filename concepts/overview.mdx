...
People and sources are the shared inputs both of them draw on. Read the tree top to bottom;
...
│       │                  chatbot endpoint people experience.
│       └── run            a group of simulated people dispatched at once.
│           └── participant → reactions   one person's reported journey + the reasoning.
├── ask                    a lighter comparison: a fixed panel, no artifact.  (alias a-)
│   └── round              one dispatch of the variants to that panel.        (alias r-)
│       └── per-person reactions: a pick, optional ratings, and the why.
├── people                 the saved audience a study or an ask draws from.
└── sources                the evidence that grounds generated people.
```

A workspace is the root; a participant's reactions are the leaf of either branch. A study and an
ask never nest inside each other: they sit side by side in the workspace and share its people and
sources. The study branch is the one most of this page follows, because it is the durable one.
The ask branch swaps iterations and runs for rounds, and is covered in
