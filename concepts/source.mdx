---
title: "Sources"
description: "The real evidence behind a generated person: transcripts, audio, images, and PDFs that ground a simulated Person in how someone actually reacted."
mode: "explanation"
---
...
A brief alone produces a plausible Person. A brief plus a source produces a Person pinned to evidence. The generation job reads the file and folds what it finds into the record it builds: the vocabulary in a transcript, the objection in an email, the friction visible in a screenshot. The richer the input, the less the generated person is invented and the more it reflects a real reaction you have already seen.

You can attach more than one source to a single generation, and pair each one with a short note about how that person reacted to that artifact. The note rides along with the file so the generated Person reflects the reaction, not just the document.
...
Inline is the common path. In the CLI, `ish person generate --source ./call.mp3` uploads and processes the file before building the Person. In MCP, `person_generate(source_paths=[...])` does the same inline.
...
First, deletes are ref-counted. `ish source delete` removes the source row and its uploaded file only when no generated Person still maps to it. A person already built from a source keeps that seed mapping until the person itself is deleted, so deleting a source never silently rewrites the people it grounded. Like every removal in ish, the delete asks for confirmation and is not reversible.
...
<Columns cols={2}>
  <Card title="People and panels" icon="users" href="/concepts/people">
    What a Person is and how a source grounds the people you generate.
  </Card>
