---
title: "Benchmarks and reference workspaces"
description: "How a reference workspace and study benchmark let you run the same study head-to-head against competitors or alternative versions."
mode: "explanation"
---
...
ish models that with two pieces. A **reference workspace** is one comparison target.
A **benchmark** clones a [study](/concepts/study) across a set of references so each
...
is no CLI surface for references or benchmarks.

## A reference is a comparison workspace

A reference is a [workspace](/concepts/workspace#references-comparison-workspaces)
in disguise. Underneath it is a workspace with a parent set and a `product_type` of
...
A benchmark is also idempotent against a reference that already has a clone of this
study, and it never silently overrides access. Any reference it could not clone to
...
<Steps>
  <Step title="Create one reference per comparison target">
...
<Warning>
  Deleting a reference cascades to every study and iteration cloned under it, with
  no soft archive. Confirm before calling `reference_delete`. The widest destructive
  action, deleting the parent workspace, takes its references with it. See the
