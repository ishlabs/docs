...
`workspace_get` lists only standard workspaces, never brands. `reference_get` lists
...
[reference tools](/mcp/generated/tools-reference) for `reference_create` /
`reference_get` / `reference_delete`, and [study tools](/mcp/generated/tools-study) for
...
<Warning>
  ...
  soft archive. Confirm before calling `reference_delete`. The widest destructive
  action, deleting the parent workspace, takes its brands with it. See the
  [reference tools](/mcp/generated/tools-reference).
</Warning>
...
<Columns cols={3}>
  ...
  <Card title="reference tools" icon="/images/logos/mcp.svg" href="/mcp/generated/tools-reference">
    The MCP reference for `reference_create`, `reference_get`, and `reference_delete`.
  </Card>
