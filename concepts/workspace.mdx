...
the sources you upload, secrets, and the references you compare against. Pick a
...
A reference is a workspace in disguise. When you want to benchmark your study against
a competitor or an alternative version on the same panel, you create a reference under
a parent workspace. At the data layer a reference is a workspace with a parent set and
a `product_type` of `reference`; ish gives it its own `reference` namespace so you
...
[iteration](/concepts/iteration) to fill in with that reference's URL or media, so the
...
<Columns cols={3}>
  ...
  <Card title="Benchmarks and references" icon="code-compare" href="/concepts/benchmarking">
